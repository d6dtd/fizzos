...
#define _syscall4(type, name, atype, a, btype, b, ctype, c, dtype, d)                                   \
    ...
    {                                                                                         \
        ...
        __asm__ volatile("int $0x80"                                                          \
                         : "=a"(__res)                                                        \
                         : "0"(__NR_##name), "b"((int)(a)), "c"((int)(b)), "d"((int)(c)), "S"((int)(d))); \
        if (__res >= 0)                                                                       \
            return (type)__res;                                                               \
        errno = -__res;                                                                         \
        return -1;                                                                            \
...
int close(int fd); // 6
int execve(const char* pathname, const char* const* argv, const char* const* envp); //11
int chdir(const char *path);
pid_t getpid(void);
int mkdir(const char *pathname, mode_t mode);
int getcwd(char* buf, size_t size);
int mkdirat(int dirfd, const char *pathname, mode_t mode); // if absolute ignore dirfd, else reletive of dirfd
int openat(int dfd, const char* filename, int flags, mode_t mode); // 243
